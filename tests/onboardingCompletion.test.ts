...
const withoutHandle = { ...baseState, handle: '' };
assert.equal(
  hasCompletedOnboarding(withoutHandle),
  false,
  'should require a handle for onboarding to be complete',
);

const missingRequiredField = { ...withPreferredSendTimeOptOut, education_level: '' };
assert.equal(
  hasCompletedOnboarding(missingRequiredField),
  false,
  'should require every mandatory field even when preferred send time is opted out',
);
